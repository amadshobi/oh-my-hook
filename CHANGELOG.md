...
- **Standard JSON Schema Lowercase Type Normalization**:
  - Normalized JSON Schema `type` keywords to canonical lowercase (`string`, `object`, `array`, `boolean`, `integer`, `number`) in `gateway/antigravity.js` conforming strictly to JSON Schema draft-07 and AJV validator expectations, resolving 400 bad request errors from CommandCode proxy during subagent spawning.
  - Updated gateway tests in `tests/gateway.test.js` to assert lowercase schema types across primitive, array, and nested object declarations.
...
- **Bounded Session Maps & Memory Hygiene (Issue #14)**:
  - Capped module-level `pushMilestones` and `sessionAutoCompactState` Maps to maximum 100 sessions with automatic FIFO eviction of oldest entries, preventing silent memory leaks in long-lived OpenCode server processes.
  - Added `session.deleted` event cleanup handler in `compress/automation.js` to immediately purge automation and idle state when a session is destroyed.
  - Added dedicated unit tests in `tests/compress-automation.test.js` validating session deletion cleanup and bounded capacity eviction.
