...
import { refreshReads } from "../guard/read-guard.js";
import { statOf } from "../share/state.js";
import { filePathOf } from "../share/hook.js";
...
function runQuiet(cmd, args, opts = {}) {
  try {
    execFileSync(cmd, args, { stdio: "ignore", timeout: opts.timeout ?? 15000, cwd: opts.cwd });
    return true;
  } catch {
    return false;
  }
...
function runTypeCheck(filePath) {
  ...
  const localTsc = path.join(tsconfigDir, "node_modules", ".bin", process.platform === "win32" ? "tsc.cmd" : "tsc");
  const cmd = existsSync(localTsc) ? localTsc : "npx";
  const args = existsSync(localTsc) ? ["--noEmit", "--pretty"] : ["tsc", "--noEmit", "--pretty"];

  const result = run(cmd, args, { timeout: 60000, cwd: tsconfigDir, silent: true });
...
function findTestFile(filePath) {
  ...
  const patterns = [
    ...
    path.join(dir.split(path.sep).join("/").replace("/src/", "/tests/").split("/").join(path.sep), `test_${base}.py`),
  ];
  ...
  return {
    "tool.execute.after": async (input, output) => {
      ...
      if (linter) {
        ...
        const result = run(cmd, [...argsList, filePath], { timeout: 15000, cwd, silent: true });
        if (!result.ok) {
          output.metadata = {
            ...(output.metadata || {}),
            lint: { status: "issues", output: (result.error || "").slice(0, 500) },
          };
      ...
      if (fixer) {
        ...
        const ok = runQuiet(cmd, [...argsList, filePath], { timeout: 15000, cwd });
        if (ok) {
      ...
      if (testFile) {
        ...
        if (runner) {
          ...
          const result = run(cmd, [...argsList, testFile], { timeout: 30000, cwd, silent: true });
          if (!result.ok) {
            output.metadata = {
              ...(output.metadata || {}),
              tests: { status: "fail", output: (result.error || "").slice(0, 500) },
            };
          }
        }
      }
      ...
      if ([".ts", ".tsx", ".js", ".jsx", ".css", ".scss"].includes(ext)) {
        try {
          ...
          if (buildScript) {
            ...
            if (distDir) {
              const sizeMB = parseFloat((dirSize(distDir) / 1024 / 1024).toFixed(2));
              if (sizeMB > BUNDLE_THRESHOLD_MB) {
                output.metadata = {
                  ...(output.metadata || {}),
                  bundle: { status: "warning", size: `${sizeMB}MB`, threshold: `${BUNDLE_THRESHOLD_MB}MB` },
                };
                await notify(`Bundle size ${sizeMB}MB exceeds ${BUNDLE_THRESHOLD_MB}MB threshold`);
              }
            }
          }
        } catch {}
