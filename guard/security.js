/**
 * guard/security.js — security & safety guardrails, ported from the old
 * plugins/guardrails.js: secret scanner, dangerous bash, commit guard,
 * dev-server guard, push warnings, stray .md warnings.
 */
import path from "node:path";
import { formatBlockMessage, formatWarnMessage } from "../share/messages.js";
import { toolArgs, bashCommand, filePathOf, writeContent } from "../share/hook.js";
...
const SECRET_PATTERNS = [
  ...
  { name: "GitHub Token", regex: /(ghp|gho|ghu|ghs|ghr|github_pat)_[A-Za-z0-9_]{20,}/g },
...
function dangerousBashPatterns(command) {
  return DANGEROUS_PATTERNS.some((re) => re.test(command));
}

const DEV_SERVER_PATTERNS = [
  "npm run dev", "npm start", "yarn dev", "pnpm dev", "bun dev", "next dev",
  "vite", "webpack serve", "nodemon", "ts-node-dev",
  "python manage.py runserver", "flask run", "uvicorn", "cargo watch",
];
...
const MD_ALLOWED_PATTERNS = [
  "/docs/", "/documentation/", "AGENTS.md", "README.md", "CHANGELOG.md",
  "CONTRIBUTING.md", "LICENSE.md", "OPENCODE.md", "CODEMAP.md", "SKILL.md",
  "/commands/", "/skills/", "/agents/", "/rules/", "/templates/",
];

const CONVENTIONAL_RE = /^(feat|fix|docs|style|refactor|perf|test|chore|ci|build|revert)(\(.+\))?!?:\s.+/;
...
function checkCommitMessage(command) {
  ...
  return errors.length > 0
    ? { block: true, reason: "Commit message issues:\n  - " + errors.join("\n  - ") }
    : null;
}

function checkPush(command) {
  if (!command.startsWith("git push")) return null;
  const isForce = command.includes("--force") || command.includes("-f");
  const isMain = command.includes("main") || command.includes("master");

  if (isMain && isForce) {
    return {
      block: true,
      reason: "Force pushing to main/master is strictly blocked. Consider creating a PR instead.",
    };
  }
  ...
  return warnings.length > 0 ? { block: false, warning: warnings.join(" | ") } : null;
}
...
export const securityHooks = async ({ client }, opts = {}) => {
  ...
  const notify = createNotifier(client, "guardrails", "warn");
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (secretScanner && (tool === "write" || tool === "edit" || tool === "patch")) {
        const filePath = filePathOf(args);
        const content = writeContent(args);
        if (filePath || content) {
          const findings = scanContentForSecrets(content);
          if (findings.length > 0) {
            const detail = findings.map((f) => `  - Line ${f.line}: ${f.type}`).join("\n");
            await notify(`Potential secrets detected in ${path.basename(filePath) || "content"}`);
            throw new Error(
              formatBlockMessage(
                "secretDetected",
      ...
      if (tool === "bash") {
        ...
        if (dangerousBash && dangerousBashPatterns(command)) {
          await notify(`Blocked dangerous bash command: ${command.slice(0, 120)}`);
          throw new Error(
            formatBlockMessage(
              "dangerousBash",
