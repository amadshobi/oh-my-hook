/**
 * context/context.js — session context, compaction snapshot, prompt check,
 * ported from the old plugins/context.js. Injects git status + pending
 * todos into the compaction prompt so the model doesn't lose context
 * after compaction.
 */
...
function gitSnapshot(cwd) {
  ...
  try {
    ...
    const status = execFileSync("git", ["status", "--short"], { cwd, stdio: "pipe" }).toString().trim();
...
function pendingTodos(cwd) {
  ...
  try {
    ...
    return pending.map((t) => `  - [ ] ${t.task || t.content || JSON.stringify(t)}`).join("\n");
  } catch {
    return null;
  }
}
...
export const contextHooks = async ({ client, directory }, opts = {}) => {
  const cfg = opts?.config ?? {};
  const compactionSnapshot = cfg.compactionSnapshot ?? true;
  const promptCheck = cfg.promptCheck ?? true;
  const compactThreshold = cfg.compactThreshold ?? 50;

  const notify = createNotifier(client, "context", "info");

  const editCounter = { count: 0 };

  const remindersFile = path.join(directory || os.homedir(), ".opencode", "reminders.md");
  const readReminders = () => {
    try {
      if (existsSync(remindersFile)) return readFileSync(remindersFile, "utf8").trim();
    } catch {}
    return "";
  };
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        const sessionInfo = { packageManager: pkgManager, startedAt: new Date().toISOString() };
        if (lastSession) {
      ...
      if (event.type === "session.idle" || event.type === "session.deleted") {
        ...
        try {
          ...
          try {
            const log = execFileSync("git", ["log", "--oneline", "-5", "--since=4 hours ago"], { cwd, stdio: "pipe" }).toString().trim();
      ...
      try {
        ...
        if (typeof text === "string" && text.trim()) {
          ...
          if (analysis.warnings.length > 0) {
            await notify(`Prompt check (${analysis.wordCount} words): ${analysis.warnings.join(" | ")}`);
          }
        }
      } catch {}
    },
    ...
    "tool.execute.after": async (input) => {
      ...
      if (editCounter.count > 0 && editCounter.count % compactThreshold === 0) {
        await notify(
          `You have made ${editCounter.count} edits this session. Consider running /compact to free up context window space.`
        );
      }

      if (editCounter.count > 5) {
        const hasTestCmd = ["package.json", "pyproject.toml", "Makefile"].some((f) => existsSync(path.join(cwd, f)));
        if (hasTestCmd) {
          await notify(`${editCounter.count} files modified. Consider running the test suite before wrapping up.`);
        }
      }
    },
