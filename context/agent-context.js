/**
 * context/agent-context.js — inject file/URL-based context into the system
 * prompt, gated by agent mode (main vs subagent).
 *
 * Config shape (from omh.jsonc):
 *   context.agent.main:     string[]  (files/URLs for primary agents)
 *   context.agent.subagent: string[]  (files/URLs for subagents)
 *
 * Paths support:
 *   - absolute: /home/user/rules.md
 *   - ~/...   : home-relative
 *   - relative: resolved against the project directory (cwd)
 *   - http(s): fetched (cached + short timeout; failures are skipped)
 */
...
/** Resolve a local path against the project directory. */
...
/**
 * Create the agent-context hooks. `cfg` is `config.context`.
 */
export const agentContextHooks = async ({ directory }, opts = {}) => {
  const cfg = opts?.config ?? {};
  const agentCfg = cfg.agent ?? {};
  const enabled = cfg.agentEnabled ?? true;
  ...
  return {
    ...
    "experimental.chat.system.transform": async (input, output) => {
      if (!enabled) return;
      const sources = isSubagent(input, agentModes) ? subagentSources : mainSources;
      if (!Array.isArray(sources) || sources.length === 0) return;
