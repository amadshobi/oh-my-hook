...
export const MEMORY_ROOT = path.join(os.homedir(), ".config", "opencode", "memory");
...
export function projectSlug(projectDir) {
  ...
  const normalized = path.resolve(projectDir).split(path.sep).join("/").replace(/^\/+/, "");
  return normalized;
}

export function projectMemoryFile(projectDir) {
  return path.join(MEMORY_ROOT, "projects", projectSlug(projectDir), "MEMORY.md");
}
