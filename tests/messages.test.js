...
test("interpolate replaces placeholders correctly", () => {
  ...
  const res = interpolate(tpl, { tool: "edit", file: "foo.js", reason: "syntax error" });
...
test("resolveMessage supports structured object override", () => {
  ...
  const msg = resolveMessage("secretDetected", { detail: "Line 10: GitHub Token" }, config);
...
test("resolveMessage supports external file override via {file:...}", () => {
  ...
  try {
    ...
    const msg = resolveMessage("modePlanTool", { tool: "write", target: "index.js" }, config);
...
test("formatBlockMessage and formatWarnMessage output proper guardrail formatting", () => {
  const block = formatBlockMessage("commitGuard", { reason: "Need feat/fix prefix" });
  assert.ok(block.startsWith("GUARDRAIL BLOCK: Commit Diblokir"));
  assert.ok(block.includes("Need feat/fix prefix"));

  const warn = formatWarnMessage("strayMarkdown", { file: "notes.md" });
  assert.ok(warn.startsWith("GUARDRAIL WARNING: File Markdown di Luar Docs"));
