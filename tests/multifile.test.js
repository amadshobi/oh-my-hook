...
test("omh.yaml loads and merges", () => {
  ...
  try {
    ...
    assert.equal(config.guard.planMode, false);
    assert.equal(config.guard.staleWrite, true, "defaults preserved");
  } finally {
...
test("omh.yml loads and merges", () => {
  const saved = snapshot();
  try {
    writeFileSync(configPath("yml"), `context:\n  compactThreshold: 25\nreminder:\n  checklist: false\n`);
    const { config, source } = loadConfig();
...
test("omh.json loads and merges", () => {
  const saved = snapshot();
  try {
    writeFileSync(configPath("json"), `{"guard": {"readBeforeWrite": false}, "memory": {"maxBullets": 7}}`);
    const { config, source } = loadConfig();
    assert.ok(source?.endsWith("omh.json"), `expected omh.json, got ${source}`);
    assert.equal(config.guard.readBeforeWrite, false);
...
test("omh.jsonc loads with comments", () => {
  const saved = snapshot();
  try {
    writeFileSync(configPath("jsonc"), `{\n  // komentar\n  "memory": { "enabled": false },\n  "guard": { "dangerousBash": false }\n}`);
    const { config, source } = loadConfig();
    assert.ok(source?.endsWith("omh.jsonc"), `expected omh.jsonc, got ${source}`);
    assert.equal(config.memory.enabled, false);
    assert.equal(config.guard.dangerousBash, false);
  } finally {
...
test(".jsonc beats .yaml when both exist (priority)", () => {
  ...
  try {
    ...
    const { config, source } = loadConfig();
    assert.ok(source?.endsWith("omh.jsonc"), `priority should pick jsonc, got ${source}`);
