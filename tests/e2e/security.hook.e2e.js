...
try {
  ...
  const secrets = [
    { type: "GitHub PAT", content: 'const token = "ghp_1234567890abcdefghijklmnopqrstuvwxyz1234";' },
    { type: "OpenAI Key", content: 'const key = "sk-proj-1234567890abcdefghijklmnopqrstuvwxyz1234";' },
    { type: "AWS Access Key", content: 'const aws = "AKIAIOSFODNN7EXAMPLE";' },
    { type: "Database URL", content: 'const db = "postgres://user:password123@localhost:5432/mydb";' },
  ];

  for (const secret of secrets) {
    let blockedWrite = false;
    try {
      await before({ tool: "write", args: { filePath: "/tmp/app.js", content: secret.content } });
    } catch (e) {
      blockedWrite = true;
      assert(/Secret Terdeteksi/i.test(e.message), "Error should mention secret detected");
    }
    assert(blockedWrite, `Expected write containing ${secret.type} to be blocked`);

    let blockedEdit = false;
    try {
      await before({ tool: "edit", args: { filePath: "/tmp/app.js", newString: secret.content } });
    } catch (e) {
      blockedEdit = true;
      assert(/Secret Terdeteksi/i.test(e.message), "Error should mention secret detected");
    }
    assert(blockedEdit, `Expected edit containing ${secret.type} to be blocked`);
  }
  ...
  for (const cmd of dangerousCommands) {
    ...
    try {
      ...
      blocked = true;
      assert(/Perintah Berbahaya/i.test(e.message), "Error should mention dangerous command");
    }
  ...
  try {
    ...
    assert(/Dev Server/i.test(e.message), "Error should mention dev server block");
  } finally {
  ...
  try {
    await before({ tool: "bash", args: { command: "tmux new-session -d -s web 'npm run dev'" } });
  } catch {
  ...
  try {
    await before({ tool: "bash", args: { command: "git commit -m 'feat(auth): implement token refresh' -m 'Co-authored-by: OpenCode <noreply@opencode.ai>'" } });
  } catch {
  ...
  try {
    ...
    assert(/Force pushing to main|Git Push/i.test(e.message), "Error should mention force push blocked");
  }
