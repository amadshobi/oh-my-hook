...
try {
	...
	const p規Hooks = await planHooks(
		{ client: null, directory: tmpDir },
		{ config: customConfig },
	);

	const commandBefore = p規Hooks["command.execute.before"];
	const toolBefore = p規Hooks["tool.execute.before"];
	...
	await commandBefore(
		{
			command: "plan",
			arguments: "to-file feature-xyz fokus ke security",
	...
	assert(
		stateAfterPlan[TEST_SESSION]?.planFile ===
			path.join(tmpDir, "feature-xyz.md"),
		"Target plan file should be recorded",
	);
	assert(planOutput.parts.length > 0, "Prompt parts should be generated");
	assert(
		planOutput.parts[0].text.includes("Mode Plan"),
		"Prompt should contain Plan mode template",
	);
	...
	assert(
		!planWriteBlocked,
		"Writing to whitelisted plan file should be ALLOWED",
	);
	console.log("  → Plan file write allowed successfully");

	// 3. Test Project Code: write to src/app.js is BLOCKED
	console.log(
		"  3. Verifying write to regular project file is BLOCKED in Plan Mode...",
	);
	...
	assert(
		projectWriteBlocked,
		"Writing to regular project file must be BLOCKED",
	);
	...
	assert(
		stateAfterApprove[TEST_SESSION]?.mode === "execute",
		"Session mode should switch to 'execute'",
	);
	assert(
		approveOutput.parts[0].text.includes("Mode Eksekusi"),
		"Approve prompt should be injected",
	);
	console.log("  → Mode switched to 'execute'");

	// 5. Test Project Code now ALLOWED in Execute Mode
	console.log(
		"  5. Verifying write to regular project file is now ALLOWED in Execute Mode...",
	);
