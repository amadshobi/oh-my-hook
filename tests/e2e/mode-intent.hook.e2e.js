...
try {
  const hooks = await modeHooks({ client: null }, { config: { planMode: true } });
  ...
  const mutatingCalls = [
    { tool: "edit", args: { filePath: "/tmp/foo.js", oldString: "a", newString: "b" } },
    ...
    try {
      ...
      blocked = true;
      assert(/Plan Mode/i.test(e.message), `Expected plan block error, got: ${e.message}`);
    }
    assert(blocked, `Expected mutating call (${call.tool}: ${JSON.stringify(call.args)}) to be blocked`);
  }
  ...
  await before({ tool: "read", args: { filePath: "/tmp/foo.js" }, sessionID: TEST_SESSION });
  await before({ tool: "glob", args: { pattern: "*.js" }, sessionID: TEST_SESSION });
  await before({ tool: "bash", args: { command: "git status" }, sessionID: TEST_SESSION });
  await before({ tool: "bash", args: { command: "ls -la" }, sessionID: TEST_SESSION });
  ...
  assert(stateAfterExec[TEST_SESSION]?.mode === "execute", "Session mode should be 'execute'");
  ...
  await before({ tool: "edit", args: { filePath: "/tmp/foo.js" }, sessionID: TEST_SESSION });
  await before({ tool: "bash", args: { command: "git commit -m 'feat: ok'" }, sessionID: TEST_SESSION });
